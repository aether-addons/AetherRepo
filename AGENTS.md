...
Source/development lives in sibling repo: https://github.com/aether-addons/AetherScraper.

## Required AI workflow

1. Treat this repo as generated release output, not source development workspace.
2. Make add-on code changes in `../AetherScraper` first.
...
10. Commit/push `AetherScraper` source changes first, then commit/push this regenerated repo output.
11. If adding new add-ons, add source folder under `AetherScraper`, then include with `--addon addon.id` or update `DEFAULT_ADDONS` in `build_repo.py`.
